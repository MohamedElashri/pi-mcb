/**
 * Tests for /mcb-recall command — search session history.
 */
...
const testRoot = join(
  tmpdir(),
  `pi-mcb-recall-test-${process.pid}-${Date.now()}`,
);
...
import { registerVccRecallCommand } from "../src/commands/mcb-recall.js";
...
describe("/mcb-recall command", () => {
  ...
  it("registers the mcb-recall command", () => {
    ...
    expect(callArgs[0]).toBe("mcb-recall");
  ...
  it("shows recent entries when no query is provided", async () => {
    ...
    await handlerMap.get("mcb-recall")!("", ctx);

    expect(sentMessages).toHaveLength(1);
    expect(sentMessages[0].customType).toBe("mcb-recall");
  ...
  it("searches and returns matches for a query", async () => {
    ...
    await handlerMap.get("mcb-recall")!("login", ctx);
  ...
  it("returns empty when no matches found", async () => {
    ...
    await handlerMap.get("mcb-recall")!("nonexistent", ctx);
  ...
  it("shows error when no session file is available", async () => {
    ...
    await handlerMap.get("mcb-recall")!("test", ctx);
  ...
  it("uses in-memory entries while Pi has not created the session file", async () => {
    const { pi, handlerMap, sentMessages } = createMockEnvironment();
    registerVccRecallCommand(pi as any);

    const ctx: any = {
      sessionManager: {
        getSessionFile: vi.fn(() => join(testRoot, "not-yet-flushed.jsonl")),
        getBranch: vi.fn(() => [{ id: "m0" }]),
        getEntries: vi.fn(() => [
          {
            type: "message",
            id: "m0",
            message: { role: "user", content: "Memory only session entry" },
          },
        ]),
      },
      ui: { notify: vi.fn() },
    ...
    await handlerMap.get("mcb-recall")!("memory", ctx);

    expect(sentMessages).toHaveLength(0);
    expect(ctx.ui.notify).toHaveBeenCalledWith(
      expect.stringContaining("Memory only session entry"),
      "info",
    );
  });
  ...
  it("scope:all includes off-lineage results", async () => {
    ...
    await handlerMap.get("mcb-recall")!("Secret scope:all", ctx);
  ...
  it("pagination via page:N works", async () => {
    ...
    await handlerMap.get("mcb-recall")!("test page:2", ctx);
