...
const testDir = join(tmpdir(), `pi-mcb-modal-test-${Date.now()}`);
...
describe("openSettings globalConfigDir forwarding", () => {
  it("forwards the ConfigManager configDir to the modal (no extensions-dir fallback)", async () => {
    ...
    const configDir = join(testDir, "pi-mcb");
  ...
  it("openMcbSettings resolves GLOBAL_CONFIG_DIR under the agent dir", async () => {
    // mcb-settings.ts computes GLOBAL_CONFIG_DIR at module scope,
    // so it must be imported after PI_CODING_AGENT_DIR is set.
    const { openMcbSettings } = await import("../src/pi-base/mcb-settings.js");

    await openMcbSettings(ctx);

    expect(capturedOptions).toHaveLength(1);
    const opts = capturedOptions[0];
    expect(opts.globalConfigDir).toBe(join(testDir, "pi-mcb"));
  });
});
