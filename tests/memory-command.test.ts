/**
 * Tests for the /mcb-memory command (status, view, full).
 */
...
describe("/mcb-memory command", () => {
  ...
  it("shows status with pipeline counters for default config", async () => {
    ...
    await handlerMap.get("mcb-memory")!([], {
      cwd: "/tmp/test",
      sessionManager: {
  ...
  it("view mode renders visible observations and reflections", async () => {
    ...
    await handlerMap.get("mcb-memory")!(["view"], {
      cwd: "/tmp/test",
      sessionManager: {
        getBranch: vi.fn(() => entries),
        getSessionId: vi.fn(() => "test-session"),
    ...
    expect(msg).toContain("Reflections");
    expect(msg).toContain("Observations");
    expect(msg).toContain("Add `copy` to copy the complete result.");
  ...
  it("full mode renders all recorded observations and reflections", async () => {
    ...
    await handlerMap.get("mcb-memory")!(["full"], {
      cwd: "/tmp/test",
      sessionManager: {
        getBranch: vi.fn(() => entries),
        getSessionId: vi.fn(() => "test-session"),
  ...
  it("shows usage message for invalid mode", async () => {
    ...
    await handlerMap.get("mcb-memory")!(["invalid-mode"], {
      cwd: "/tmp/test",
      sessionManager: {
        getBranch: vi.fn(() => []),
        getSessionId: vi.fn(() => "test-session"),
