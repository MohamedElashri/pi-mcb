/**
 * Tests for mcb's debug-log (single-file rotation approach).
 *
 * Upstream pi-observational-memory uses per-session debug files; ours is a
 * single ndjson file at ~/.pi/agent/pi-mcb/debug.ndjson with rotation.
 */
...
const testRoot = join(
  tmpdir(),
  `pi-mcb-debug-test-${process.pid}-${Date.now()}`,
);
...
describe("debug-log", () => {
  ...
  it("writes to debug.ndjson path", () => {
    ...
    flushDebugLog();
    expect(existsSync(join(agentDir, "pi-mcb", "debug.ndjson"))).toBe(true);
  });
  ...
  it("exports known constants", () => {
    ...
    expect(DEBUG_LOG_RELATIVE_PATH).toBe(join("pi-mcb", "debug.ndjson"));
  });
});
