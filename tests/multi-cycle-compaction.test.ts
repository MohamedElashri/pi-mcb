...
describe("T41: 3 auto-compact cycles with pi-default", () => {
  test("messages compiled in cycle 1 are NOT recompiled in cycle 2", () => {
    ...
    const afterCycle1 = [
      comp("c1", "m5"), // mcb's compaction marker (not real pi format, but close enough)
      msg("m5", "user", "e"),
      msg("m6", "assistant", "f"),
      msg("m7", "user", "g"),
      msg("m8", "assistant", "h"),
...
// ---------------------------------------------------------------------------
// T42: /mcb (minimal) after pi-default auto-compact
// ---------------------------------------------------------------------------

describe("T42: /mcb after pi-default auto-compact", () => {
  test("/mcb compiles everything since last compaction", () => {
    ...
    // Now user runs /mcb (minimal) on the remaining branch
    const afterAuto = [
      comp("c1", "m5"),
      msg("m5", "user", "e"),
      msg("m6", "assistant", "f"),
    ...
    // /mcb uses minimal → finds last user (m7), compiles everything before
  ...
  test("/mcb after pi-default with orphan recovery still works", () => {
    ...
    // Simulate after compact-all, then more chat, then /mcb
    const afterCompact = [
      msg("old1", "user", "old"),
      comp("c1", ""), // compact-all sentinel
      msg("m1", "user", "a"),
    ...
    // /mcb (minimal) with orphan recovery
    const r = buildOwnCut(
      afterCompact,
      undefined, // no Pi cut
      "minimal",
...
describe("T43: Switch tailBehavior mid-session", () => {
  ...
  test("effective tail behavior in hook changes per invocation (isMcb flag)", () => {
    // Hook resolves effectiveTailBehavior per call:
    //   /mcb → minimal (even if config says pi-default)
    ...
    // Simulate auto-triggered: isMcb=false → effective=pi-default
    ...
    const mcbResult = buildOwnCut(
      ...
      "m3",
      "minimal", // effective tail behavior for /mcb
    );
    expect(mcbResult.ok).toBe(true);
    if (!mcbResult.ok) return;
    // Both produce the same result here because m3 is the last user
    expect(mcbResult.firstKeptEntryId).toBe("m3");
  });
});
