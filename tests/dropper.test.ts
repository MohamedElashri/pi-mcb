 ...
 * Ported with adaptations for pi-mcb:
...
describe("V3 dropper agent", () => {
  ...
  it("computes max drops from pool fullness ratio (mcb algorithm)", () => {
  ...
  it("passes budget-return max drops as a hard upper bound", async () => {
    ...
    // pi-mcb display format: "fullness: ~X%" (no "against target")
    ...
    // pi-mcb still uses DropUrgency
    expect(userText).toContain("Drop urgency");
    // pi-mcb doesn't display "over target by ~X tokens" or "sized to move..."
