...
const testDir = join(tmpdir(), `pi-mcb-config-${randomUUID().slice(0, 8)}`);
...
function configPath(): string {
  return join(testDir, "pi-mcb", "pi-mcb-config.json");
}
...
afterEach(() => {
  ...
  delete process.env.PI_MCB_PASSIVE;
  delete process.env.PI_MCB_OM_PASSIVE;
  delete process.env.PI_OBSERVATIONAL_MEMORY_PASSIVE;
  delete process.env.PI_MCB_COMPACTION;
  delete process.env.PI_MCB_COMPACTION_ENGINE;
  try {
...
describe("Old → new key migration", () => {
  it("T2: overrideDefaultCompaction:true → compactionEngine:mcb + tailBehavior:minimal", async () => {
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
  ...
  it("T6: mixed old + new — new wins, old ignored", async () => {
    ...
    expect(config.compactionEngine).toBe("mcb"); // default, not migrated
  ...
  it("T8: migration runs once — old keys deleted from parsed result", async () => {
    ...
    expect(config1.compactionEngine).toBe("mcb");
    ...
    expect(config2.compactionEngine).toBe("mcb");
...
describe("Env overrides", () => {
  it("T9: PI_MCB_PASSIVE=1 forces compaction:off + memory:false", async () => {
    process.env.PI_MCB_PASSIVE = "1";
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
  ...
  it("PI_MCB_PASSIVE is deprecated but still works with legacy name PI_MCB_OM_PASSIVE", async () => {
  ...
  it("PI_MCB_COMPACTION env var overrides compaction", async () => {
    process.env.PI_MCB_COMPACTION = "manual";
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
  ...
  it("PI_MCB_COMPACTION_ENGINE env var overrides compactionEngine", async () => {
    process.env.PI_MCB_COMPACTION_ENGINE = "pi-default";
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
    writeConfig({ compactionEngine: "mcb" });
...
describe("scaffoldConfig — NixOS safety", () => {
  ...
  it("does not crash on read-only filesystem during scaffold", async () => {
    ...
    mkdirSync(join(testDir, "pi-mcb"), { recursive: true });
    try {
      chmodSync(join(testDir, "pi-mcb"), 0o555);
    } catch {
      /* skip on Windows */
    }
    ...
    // Restore permissions so afterEach cleanup works
    try {
      chmodSync(join(testDir, "pi-mcb"), 0o755);
    } catch {
      /* skip on Windows */
    }
  });
