/**
 * Tests for /mcb command — compaction trigger, om-off/om-on, noAutoCompact flush.
 */
...
const { testRoot } = vi.hoisted(() => {
  ...
  return {
    testRoot: join(tmpdir(), `pi-mcb-cmd-test-${process.pid}-${Date.now()}`),
  };
});
...
import { registerMcbCommand } from "../src/commands/pi-mcb.js";
...
describe("/mcb command", () => {
  beforeEach(() => {
    mkdirSync(join(testRoot, "agent", "pi-mcb"), { recursive: true });
  });
  ...
  it("registers the mcb command", () => {
    const { pi, runtime } = createMockEnvironment();
    registerMcbCommand(pi as any, runtime as any);
  ...
  it("surfaces a single 'settings' completion (with 'configure' alias matching)", () => {
    ...
    const completions = completionMap.get("mcb")!("");
    const values = completions.map((c) => c.value);
    expect(values).toContain("settings");
    expect(values).not.toContain("configure");

    // Typing /mcb config… surfaces the settings entry via its alias
    const configMatches = completionMap.get("mcb")!("config").map(
      (c) => c.value,
    );
    expect(configMatches).toEqual(["settings"]);
  });

  it("calls ctx.compact with PI_MCB_COMPACT_INSTRUCTION", async () => {
    const { pi, runtime, handlerMap, makeHandlerArgs } =
      createMockEnvironment();
    registerMcbCommand(pi as any, runtime as any);

    const ctx = makeHandlerArgs();
    await handlerMap.get("mcb")!("", ctx);
  ...
  it("/mcb om-off disables memory and saves config", async () => {
    const { pi, runtime, handlerMap, makeHandlerArgs, notifyCalls } =
      createMockEnvironment();
    registerMcbCommand(pi as any, runtime as any);
  ...
  it("/mcb om-on enables memory and saves config", async () => {
    ...
    const ctx = makeHandlerArgs();
    await handlerMap.get("mcb")!("om-on", ctx);
  ...
  it("flush pending entries when noAutoCompact is active and pending data exists", async () => {
    ...
    const pendingDir = join(testRoot, "agent", "pi-mcb");
...
describe("/mcb follow-up prompt", () => {
  ...
  it("extracts follow-up text from /mcb <args> and sends it after compaction", async () => {
    ...
    const ctx = makeHandlerArgs();
    await handlerMap.get("mcb")!("fix the auth bug", ctx);
  ...
  it("does NOT extract subcommands as follow-up", async () => {
    ...
    const ctx = makeHandlerArgs();
    await handlerMap.get("mcb")!("configure", ctx);
  ...
  it("treats 'settings' as an alias for 'configure'", async () => {
    ...
    const ctx = makeHandlerArgs();
    await handlerMap.get("mcb")!("settings", ctx);
  ...
  it("fires follow-up via sendUserMessage after compaction completes", async () => {
    ...
    const ctx = makeHandlerArgs();
    await handlerMap.get("mcb")!("continue the refactor", ctx);
  ...
  it("does not fire follow-up when compaction fails", async () => {
    ...
    const ctx = makeHandlerArgs();
    await handlerMap.get("mcb")!("continue", ctx);
