 ...
 *   - If it fires, which pipeline runs (mcb vs Pi default)?
...
// Branch with enough messages (>2 live) to pass buildOwnCut for the mcb pipeline
  ...
  for (const tc of guardCases) {
    it(`${tc.name}: ${tc.expectFire ? "fires" : "blocks"} auto-compaction`, async () => {
      ...
      if (tc.expectFire) {
        ...
        expect(
          system.notifyCalls.some((n) =>
            n.msg.includes("compaction threshold reached"),
          ),
        ).toBe(true);
        // Should show completion notification
        expect(
          system.notifyCalls.some((n) => n.msg.includes("compaction complete")),
        ).toBe(true);
      } else {
...
describe("Auto-compact trigger: before-compact hook integration", () => {
  ...
  interface IntegrationCase {
    ...
    expectMcbPipeline: boolean; // does the before-compact hook run mcb pipeline?
  ...
  const integrationCases: IntegrationCase[] = [
    ...
    // overrideDefaultCompaction=true: hook runs mcb pipeline
    {
      name: "override=true, noAutoCompact=false",
      noAutoCompact: false,
      overrideDefaultCompaction: true,
      expectAutoFire: true,
      expectMcbPipeline: true,
      expectPiDefaultPipeline: false,
    },
    {
  ...
  for (const tc of integrationCases) {
    it(`${tc.name}`, async () => {
      ...
      if (tc.expectAutoFire) {
        ...
        if (tc.expectMcbPipeline) {
          // before-compact hook returned a result (not undefined — mcb handled it)
...
describe("Auto-compact trigger: deferral and re-check paths", () => {
  ...
  it("keeps waiting when agent is not idle (issue #31)", async () => {
    ...
    expect(
      system.notifyCalls.some((n) => n.msg.includes("compaction deferred")),
    ).toBe(false);
  });
  ...
  it("skips when token pressure was reduced by another compaction between trigger and microtask", async () => {
    ...
    expect(system.runtime.compactInFlight).toBe(false);
    expect(
      system.notifyCalls.some((n) => n.msg.includes("compaction skipped")),
    ).toBe(true);
  });
  ...
  it("handles session reload between trigger and microtask", async () => {
    ...
    expect(system.runtime.compactInFlight).toBe(false);
    expect(
      system.notifyCalls.some((n) => n.msg.includes("session changed")),
    ).toBe(true);
  });
});
...
describe("Auto-compact trigger: compactInFlight latch safety", () => {
  ...
  it("resets compactInFlight after microtask error", async () => {
    ...
    expect(
      system.notifyCalls.some((n) => n.msg.includes("compact threw")),
    ).toBe(true);
  });
  ...
  it("does not block subsequent agent_end when compactInFlight gets stuck", async () => {
    ...
    expect(
      system.notifyCalls.some((n) => n.msg.includes("compaction threshold")),
    ).toBe(false);
  });
});
      ...
      for (const threshold of bools) {
        ...
        it(`[${++caseIndex}/12] ${label}: ${expectFire ? "fires" : "blocks"}`, async () => {
          ...
          if (expectFire) {
            ...
            expect(
              system.notifyCalls.some((n) =>
                n.msg.includes("compaction complete"),
              ),
            ).toBe(true);
          } else {
            expect(system.ctx.compact).not.toHaveBeenCalled();
