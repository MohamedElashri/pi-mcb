...
const testDir = join(tmpdir(), `pi-mcb-config-test-${Date.now()}`);
...
function writeConfig(
  data: unknown,
  filename = "pi-mcb/pi-mcb-config.json",
): string {
...
describe("Config defaults", () => {
  it("uses all defaults when no config file exists", async () => {
    ...
    expect(config.compactionEngine).toBe("mcb");
...
describe("Legacy config fallback", () => {
  it("loads from legacy pi-mcb-config.json and migrates to new keys", async () => {
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
    writeConfig(
    ...
    const config = loadUnifiedConfig(testDir);
    // Legacy overrideDefaultCompaction:true → compactionEngine:mcb + tailBehavior:minimal
    expect(config.compactionEngine).toBe("mcb");
    expect(config.tailBehavior).toBe("minimal");
  ...
  it("loads legacy om config from settings.json under pi-mcb key and migrates", async () => {
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
    writeConfig(
      {
        "pi-mcb": {
...
describe("Env overrides", () => {
  afterEach(() => {
    delete process.env.PI_MCB_OM_PASSIVE;
    delete process.env.PI_OBSERVATIONAL_MEMORY_PASSIVE;
  });

  it("env PI_MCB_OM_PASSIVE=true forces compaction:off + memory:false", async () => {
    process.env.PI_MCB_OM_PASSIVE = "true";
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
  ...
  it("env PI_MCB_OM_PASSIVE=false undoes passive migration", async () => {
    process.env.PI_MCB_OM_PASSIVE = "false";
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
...
describe("Declarative env overrides apply at runtime", () => {
  afterEach(() => {
    ...
    delete process.env.PI_MCB_COMPACT_AFTER_TOKENS;
    delete process.env.PI_MCB_DEBUG;
    delete process.env.PI_MCB_DROPPER_PRESSURE_THRESHOLD;
    delete process.env.PI_MCB_OBSERVE_AFTER_TOKENS;
  });

  it("int override wins over the file value (runtime path)", async () => {
    process.env.PI_MCB_COMPACT_AFTER_TOKENS = "200000";
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
  ...
  it("invalid int falls back to the configured value", async () => {
    process.env.PI_MCB_COMPACT_AFTER_TOKENS = "not-a-number";
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
    writeConfig({ compactAfterTokens: 185_000 });
    const config = loadUnifiedConfig(testDir);
  ...
  it("float override (dropperPressureThreshold) applies", async () => {
    process.env.PI_MCB_DROPPER_PRESSURE_THRESHOLD = "0.5";
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
  ...
  it("out-of-range float is rejected (keeps configured value)", async () => {
    process.env.PI_MCB_DROPPER_PRESSURE_THRESHOLD = "2.0";
    const { loadUnifiedConfig } = await import("../src/core/unified-config.js");
    writeConfig({ dropperPressureThreshold: 0.7 });
    const config = loadUnifiedConfig(testDir);
    expect(config.dropperPressureThreshold).toBe(0.7);
...
describe("Integer fields are validated as positive integers", () => {
  it("rejects negative token values, falls back to defaults", async () => {
    ...
    const config = loadUnifiedConfig(testDir);
    expect(config.observeAfterTokens).toBe(15_000); // mcb default (upstream is 10_000)
    expect(config.reflectAfterTokens).toBe(25_000); // mcb default (upstream is 20_000)
