...
const testDir = join(tmpdir(), `pi-mcb-runtime-test-${Date.now()}`);
...
function cooldownFile() {
  return join(testDir, "pi-mcb", "pi-mcb-cooldown.json");
}
...
it("loads cursors from pending state", async () => {
  ...
  const pendingDir = join(testDir, "pi-mcb");
...
it("saves cursors to pending state", async () => {
  ...
  const raw = JSON.parse(
    readFileSync(join(testDir, "pi-mcb", "test-session-pending.json"), "utf-8"),
  );
