...
describe("normalize", () => {
  ...
  it("includes thinking blocks (mcb keeps them, unlike upstream)", () => {
    const blocks = normalize([assistantWithThinking("result", "hmm")]);
    // mcb keeps thinking blocks (upstream commit a156870 deferred).
  ...
  it("normalizes tool result with isError flag", () => {
    ...
    // mcb keeps isError (upstream commit a156870 deferred)
    expect(blocks).toEqual([
      {
        kind: "tool_result",
        name: "Read",
