/**
 * Ported from upstream pi-mcb
 * Changes:
 *   - bun:test → vitest, .js extensions
 *   - Adapted for mcb's registerBeforeCompactHook which requires omRuntime param
 *   - Added mockRuntime with ensureConfig that reads our config format
 *   - Removed PI_MCB_CONFIG_PATH env var (mcb uses unified config)
 */
import {
  describe,
  test,
...
beforeAll(() => {
  ...
  CONFIG_PATH = join(tmpDir, "mcb-config.json");
});
...
describe("registerBeforeCompactHook: cancel paths", () => {
  ...
  test("/pi-mcb with too few live messages cancels and notifies warning", () => {
    ...
    expect(invoke(makeEvent(entries, PI_MCB_COMPACT_INSTRUCTION))).toEqual({
      cancel: true,
    });
  ...
  test("/pi-mcb with no user message compacts all instead of cancelling", () => {
    ...
    const result = invoke(makeEvent(entries, PI_MCB_COMPACT_INSTRUCTION));
...
describe("registerBeforeCompactHook: new config key guards", () => {
  ...
  test("T28: compaction:off + /mcb → proceeds (mcb pipeline)", () => {
    ...
    // compaction: "off" allows explicit /mcb through mcb's pipeline
    expect(result.cancel).toBeUndefined();
    expect(result.compaction).toBeDefined();
  });
  ...
  test("T30: compaction:manual + /mcb → proceeds (allows manual)", () => {
  ...
  test("T32: compaction:auto + /mcb → proceeds", () => {
  ...
  test("T35: compactionEngine:pi-default + /mcb → proceeds (/mcb always uses mcb)", () => {
    const { pi, invoke, omRuntime } = createMockPi({
      compactionEngine: "pi-default",
    });
    registerBeforeCompactHook(pi, omRuntime);
...
describe("registerBeforeCompactHook: CompactionStats population", () => {
  ...
  test("populates every CompactionStats field on successful compaction", () => {
    ...
    invoke(makeEvent(entries, PI_MCB_COMPACT_INSTRUCTION));
