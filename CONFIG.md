...
Pi-mcb's configuration lives at `~/.pi/agent/pi-mcb/pi-mcb-config.json`. This document describes the new unified config keys introduced by the config simplification.
...
- The `/mcb configure` overlay shows a red error banner and **blocks Ctrl+S** until the file is fixed
...
```jsonc
{
  ...
  "compactionEngine": "mcb", // "mcb" | "pi-default"
  ...
  "onboardingSeen": true          // Set automatically after first-run preset selection
}
```

## First-run presets

In the TUI, pi-mcb asks once which starting profile to use: **Safe** (Pi's visible tail plus memory), **Aggressive** (minimal tail plus memory), or **Compaction only**. The choice is persisted as `onboardingSeen`; reset it to `false` to show the chooser again.
...
| Value | Auto-trigger | `/compact` (Pi built-in) | `/mcb` |
|-------|:---:|:---:|:---:|
| `"auto"` | mcb fires at `compactAfterTokens` threshold ✓ | mcb handles | mcb handles |
| `"manual"` | skipped | Pi handles ✓ | mcb handles |
| `"off"` | skipped (Pi handles) | Pi handles ✓ | mcb handles |
...
// Manual only — /compact falls through to Pi, /mcb uses mcb pipeline
{ "compaction": "manual" }

// Mcb skips auto + /compact (Pi handles both), /mcb still works
{ "compaction": "off" }
```

### `compactionEngine`

Controls which engine generates compaction summaries. Only meaningful when `compaction: "auto"` — for `"manual"`/`"off"` the engine is irrelevant because mcb's hook lets Pi handle everything except `/mcb`.

Replaces the old `overrideDefaultCompaction`.

| Value | Behavior |
|-------|----------|
| `"mcb"` | Mcb's `compile()` generates a structured summary and injects OM content (default). |
| `"pi-default"` | Pi handles ALL compaction (timing + execution). Mcb's trigger skips entirely. Mcb only activates for `/mcb` command. |

**Interaction matrix:**

| `compaction` | `compactionEngine` | Auto-trigger | `/compact` | `/mcb` |
|:---:|:---:|---|---|---|
| auto | mcb | mcb fires at `compactAfterTokens` ✓ | mcb handles | mcb handles |
| auto | pi-default | trigger skips (Pi decides when) | Pi handles | mcb handles |
| manual | (any) | skipped | Pi handles ✓ | mcb handles |
| off | (any) | skipped | Pi handles ✓ | mcb handles |

### `tailBehavior`

Controls how much of the recent transcript stays *visible* after compaction. Only applies when `compactionEngine: "mcb"`.
...
| `"minimal"` | Keep only the last user message. Everything before gets compiled and removed. Same as the original pi-mcb behavior (default for both auto-triggered and manual `/mcb`). |
...
|------------|:--------------------:|:---------:|
| Manual `/mcb` | not set | `"minimal"` (aggressive) |
| Manual `/mcb` | `"pi-default"` | `"pi-default"` |
...
Only applies when `compaction: "auto"` and `compactionEngine: "mcb"`.
...
Token threshold for auto-compaction. When `compaction: "auto"` and accumulated tokens since the last compaction exceed this threshold, compaction triggers automatically — both mid-run (see `midRunCompaction`) and when the agent finishes a run. If the engine is `pi-default`, mcb's trigger returns early before checking tokens.

| Type | Default |
|------|---------|
| number | 81000 |

**The interaction with Pi's threshold:** Pi has its own `keepRecentTokens` default (~20k tokens). Mcb's threshold is independent — it's the trigger point, not the keep point. When mcb's trigger fires, `tailBehavior` determines how much is actually kept visible.
...
|-----|------|---------|-------------|
| `debug` | boolean | false | Writes detailed debug snapshots to `/tmp/pi-mcb-debug.json` |
...
|----------|-----------|---------|
| `PI_MCB_COMPACTION` | `compaction` (`auto` \| `manual` \| `off`) | `PI_MCB_COMPACTION=manual` |
| `PI_MCB_COMPACTION_ENGINE` | `compactionEngine` (`mcb` \| `pi-default`) | `PI_MCB_COMPACTION_ENGINE=pi-default` |
...
| Variable | Notes |
|----------|-------|
| `PI_MCB_PASSIVE` | Current name |
| `PI_MCB_OM_PASSIVE` | Legacy pi-mcb name |
...
| `PI_MCB_FULL_FOLD_ALWAYS` | `fullFoldAlways` |

Positive-integer fields (invalid values fall back):

| Variable | Overrides |
|----------|-----------|
| `PI_MCB_COMPACT_AFTER_TOKENS` | `compactAfterTokens` |
| `PI_MCB_OBSERVE_AFTER_TOKENS` | `observeAfterTokens` |
| `PI_MCB_REFLECT_AFTER_TOKENS` | `reflectAfterTokens` |
| `PI_MCB_OBSERVATIONS_POOL_MAX_TOKENS` | `observationsPoolMaxTokens` |
| `PI_MCB_OBSERVATIONS_POOL_TARGET_TOKENS` | `observationsPoolTargetTokens` |
| `PI_MCB_REFLECTOR_INPUT_MAX_TOKENS` | `reflectorInputMaxTokens` |
| `PI_MCB_DROPPER_INPUT_MAX_TOKENS` | `dropperInputMaxTokens` |
| `PI_MCB_OBSERVER_CHUNK_MAX_TOKENS` | `observerChunkMaxTokens` |
| `PI_MCB_OBSERVER_PREAMBLE_MAX_TOKENS` | `observerPreambleMaxTokens` |
| `PI_MCB_AGENT_MAX_TURNS` | `agentMaxTurns` |

Float field (must be in `(0, 1]`):

| Variable | Overrides |
|----------|-----------|
| `PI_MCB_DROPPER_PRESSURE_THRESHOLD` | `dropperPressureThreshold` |
| `PI_MCB_DROPPER_POOL_FULLNESS_THRESHOLD` | `dropperPoolFullnessThreshold` |
...
### Pi's engine, no mcb involvement
...
- **Config file**: `~/.pi/agent/pi-mcb/pi-mcb-config.json`
- **TUI overlay**: `/mcb settings` (alias: `/mcb configure`) — opens an interactive overlay with ↑↓ navigation, Enter to toggle, Ctrl+S to save
- **CLI subcommands**: `/mcb om-off` / `/mcb om-on` — toggle memory without editing the file
