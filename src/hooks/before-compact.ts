/**
 * Before-compact hook — handles pi-mcb compaction + OM content injection.
 *
 * Upstream: https://github.com/sting8k/pi-mcb (src/hooks/before-compact.ts)
 * Modified by pi-mcb-om:
 * - After pi-mcb compiles its summary, calls buildCompactionProjection
...
export const PI_MCB_COMPACT_INSTRUCTION = "__pi_vcc__";
...
export function notifyMigrationReminder(
  ...
  notify(
    "mcb: Use `/mcb configure` to save your updated configuration.",
    "info",
  );
}
 ...
 * Example output:
 *   mcb: 6 source entries processed; tail kept 1/4 user turns (~0.5k tok).
 */
export const formatCompactionStats = (stats: CompactionStats): string => {
  ...
  return `mcb: ${parts.join("; ")} (~${formatTokens(stats.keptTokensEst)} tok).`;
};
...
const REASON_MESSAGES: Record<OwnCutCancelReason, string> = {
  no_live_messages: "mcb: Nothing to compact (no live messages)",
  too_few_live_messages:
    'mcb: Too few live messages — Pi\'s default logic preserves visible context. Set tailBehavior to "minimal" in config to force compaction with fewer messages.',
};
...
export const registerBeforeCompactHook = (
  ...
  pi.on("session_before_compact", (event, ctx) => {
    ...
    trace("before_compact.enter", {
      customInstructions,
      isMcb: customInstructions === PI_MCB_COMPACT_INSTRUCTION,
    ...
    // Always handle explicit /mcb marker.
    // Otherwise, only handle when user opted in via settings.
    const isMcb = customInstructions === PI_MCB_COMPACT_INSTRUCTION;

    // NEW: Unified compaction guards
    // compaction "off": mcb skips auto-triggered, but /mcb still uses mcb pipeline
    if (omRuntime.config.compaction === "off" && !isMcb) {
    ...
    if (omRuntime.config.compactionEngine === "pi-default" && !isMcb) {
    ...
    // compaction "manual": /compact falls through to Pi, /mcb still works
    if (omRuntime.config.compaction === "manual" && !isMcb) {
    ...
    if (
      ...
      if (!isMcb && !omRuntime.config.overrideDefaultCompaction) {
        trace("before_compact.return_early", {
          reason: "overrideDefaultCompaction=false and not /mcb",
        });
        return;
      }
      ...
      if (
        ...
        trace("before_compact.cancel", {
          reason: "manual mode and not /mcb",
        });
        return { cancel: true };
      }
    }
    ...
    // Both /mcb and auto-triggered default to "minimal" (aggressive cut);
    ...
    if (!ownCut.ok) {
      ...
      trace("before_compact.cancel", { reason: ownCut.reason, isMcb });
      try {
    ...
    const details: McbCompactionDetails = {
      compactor: "mcb",
      version: 1,
      sections: [...summary.matchAll(/^\[(.+?)\]/gm)].map((m) => m[1]),
  ...
  // /mcb path uses its own onComplete callback in the command handler.
  pi.on("session_compact", (event, ctx) => {
    if (!event.fromExtension) return;
    if (omRuntime.compactWasMcb) return; // /mcb handles its own toast via onComplete
