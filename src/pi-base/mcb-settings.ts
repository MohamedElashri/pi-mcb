/**
 * Mcb settings — modal-based configuration via ConfigManager.
 *
 * Uses pi-base's ConfigManager + openSettingsModal.
 *
...
const CONFIG_FILENAME = "pi-mcb-config.json";

export const GLOBAL_CONFIG_DIR = join(getPiAgentDir(), "pi-mcb");
...
function parseCompactionEngineEnv(
  raw: string,
): "mcb" | "pi-default" | undefined {
  const trimmed = raw.trim().toLowerCase();
  return ["mcb", "pi-default"].includes(trimmed)
    ? (trimmed as "mcb" | "pi-default")
    : undefined;
}
...
export const config = new ConfigManager<UnifiedConfig>({
  ...
  fields: (cfg) => [
    // ── Compaction ──
    {
      ...
      description:
        "auto=trigger on threshold, manual=only /mcb, off=auto:Pi handles, /mcb:mcb pipeline",
      value: cfg.compaction,
      options: ["auto", "manual", "off"],
      optionLabels: {
        auto: "auto — trigger on threshold",
        manual: "manual — only /mcb",
        off: "off — auto:Pi handles, /mcb:mcb pipeline",
      },
    },
    {
      ...
      description:
        "mcb=structured summary+OM, pi-default=built-in Pi summarization",
      value: cfg.compactionEngine,
      options: ["mcb", "pi-default"],
      optionLabels: {
        mcb: "mcb — structured summary + OM",
    ...
    // ── Debug ──
    {
      ...
      description: "Write detailed debug snapshots to /tmp/pi-mcb-debug.json",
      value: cfg.debug,
    },
    {
      key: "debugLog",
...
export async function openMcbSettings(ctx: ExtensionContext): Promise<void> {
  await config.openSettings(
    ctx,
    ctx.cwd,
    (_updated) => {
      // onSave is called after the config is persisted; caller (pi-mcb.ts)
