/**
 * Debug logging — writes JSONL to ~/.pi/agent/pi-mcb/debug.ndjson.
 *
 ...
 * Modified: path changed from observational-memory/ to pi-mcb/; async buffered.
 */
...
export const DEBUG_LOG_RELATIVE_PATH = join("pi-mcb", "debug.ndjson");
...
async function flushBuffer(): Promise<void> {
  ...
  try {
    ...
    console.error("mcb: debug log write failed", error);
  } finally {
    flushing = false;
  }
}
...
export function flushDebugLog(): void {
  ...
  try {
    ...
    console.error("mcb: debug log flush failed", error);
  }
}
