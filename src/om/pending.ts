 ...
 *
 * On manual `/mcb` trigger, pending entries are flushed to the branch
 * and the file is cleared.
 *
 * Per-session files: each session gets its own <sessionId>-pending.json
 * under ~/.pi/agent/pi-mcb/. This eliminates race conditions from
...
export interface PendingOMState {
   ...
   * branch-marker pattern. Used for LLM context and /mcb flush.
   */
  observationBatches?: PendingObservation[];
  /**
   * All reflection batches accumulated across manual mode pipeline runs.
   * Preserves per-run coverage for LLM context and /mcb flush.
   */
...
export function savePendingObservation(
  ...
  // Append to accumulated batches for LLM context and /mcb flush.
