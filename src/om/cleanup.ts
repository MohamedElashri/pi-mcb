 ...
 *
 * Per-session pending files under ~/.pi/agent/pi-mcb/ accumulate when:
...
/**
 * Scan the pi-mcb directory for all *-pending.json and *-pending.stale.json
 ...
 * pi-mcb directory.  Must be called before any unlink.
 */
function validateDeletionPaths(
  ...
  // Resolve to absolute and verify containment within pi-mcb/
 ...
 *
 * Safety: validates that both resolved paths live under the pi-mcb/
