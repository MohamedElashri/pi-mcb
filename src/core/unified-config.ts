/**
 * Unified configuration loader — merges pi-mcb + OM settings into one file.
 *
 * Created by pi-mcb-om.
 * Reads ~/.pi/agent/pi-mcb/pi-mcb-config.json with legacy fallback support.
...
const CONFIG_DIR = "pi-mcb";
const CONFIG_FILE = "pi-mcb-config.json";
...
export interface UnifiedConfig {
  ...
  /** Write debug snapshots to /tmp/pi-mcb-debug.json. */
  debug: boolean;
   ...
   *  "manual"  — only via /mcb command
   *  "off"    — never compact (disables auto + blocks /mcb) */
  compaction: "auto" | "manual" | "off";

  /** Which engine handles compaction.
   *  "mcb"  — mcb's compile() + OM injection
   *  "pi-default" — Pi's built-in summarization */
  compactionEngine: "mcb" | "pi-default";
   ...
   *  "minimal"    — keep only last user message (current agressive pi-mcb behavior)
   *  ONLY applies when compactionEngine: "mcb" */
...
const COMPACTION_ENGINE_VALUES = ["mcb", "pi-default"] as const;
...
function isCompactionEngine(v: unknown): v is "mcb" | "pi-default" {
  return (
    typeof v === "string" &&
...
function readJson(path: string): {
  ...
  try {
    ...
    const msg = `mcb: config file at ${path} has invalid JSON: ${(e as Error).message}. Using defaults.`;
    console.warn(msg);
...
/**
 * Load unified configuration from ~/.pi/agent/pi-mcb/pi-mcb-config.json.
...
export function loadUnifiedConfig(cwd: string, onWarn?: WarnFn): UnifiedConfig {
  ...
  if (!raw) {
    // Try legacy pi-mcb config
    const piVccPath = join(getAgentDir(), "pi-mcb-config.json");
    ...
    const omRaw =
      settingsRaw?.["pi-mcb"] ?? settingsRaw?.["observational-memory"];
    ...
    const projectOmRaw =
      projectRaw?.["pi-mcb"] ?? projectRaw?.["observational-memory"];
  ...
  // Project-local override: <cwd>/.pi/pi-mcb-config.json
  ...
  const envPassive =
    process.env.PI_MCB_PASSIVE ??
    process.env.PI_MCB_OM_PASSIVE ??
  ...
  const envCompaction = process.env.PI_MCB_COMPACTION;
  if (envCompaction !== undefined) {
    ...
    if (isCompaction(trimmed)) {
      ...
      console.warn(
        `mcb: invalid PI_MCB_COMPACTION value "${envCompaction}"; ignoring`,
      );
    }
  }

  // ── Env override — compaction engine ──
  const envCompactionEngine = process.env.PI_MCB_COMPACTION_ENGINE;
  if (envCompactionEngine !== undefined) {
    const trimmed = envCompactionEngine.trim().toLowerCase();
    if (isCompactionEngine(trimmed)) {
      merged.compactionEngine = trimmed as "mcb" | "pi-default";
    } else {
      console.warn(
        `mcb: invalid PI_MCB_COMPACTION_ENGINE value "${envCompactionEngine}"; ignoring`,
      );
    }
  }

  // ── Declarative PI_MCB_* overrides ──
...
export function saveUnifiedConfig(settings: Partial<UnifiedConfig>): boolean {
  try {
    ...
    if (existingResult.error) {
      console.warn("mcb: overwriting corrupt config file at " + path);
    }
 ...
 *
 * - global: writes to `<agentDir>/pi-mcb/pi-mcb-config.json`
 * - project: writes to `<cwd>/.pi/pi-mcb-config.json`
 *
...
export function saveUnifiedConfigScoped(
  ...
  try {
    const dir =
      scope === "project" ? join(cwd, ".pi") : join(getAgentDir(), "pi-mcb");
...
/**
 * Ensure ~/.pi/agent/pi-mcb/pi-mcb-config.json exists with defaults.
 *
...
export function scaffoldConfig(): void {
  try {
    ...
    console.error("mcb: config scaffold failed", e);
  }
}
 ...
 * On `/mcb`, pending entries are flushed to the branch and
