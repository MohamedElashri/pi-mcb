 ...
 *
 * Called from /mcb cleanup subcommand.
 * Scans pi-mcb pending files, cross-references against session JSONL
...
/**
 * Handle the /mcb cleanup subcommand.
 *
...
export async function handleCleanup(ctx: ExtensionContext): Promise<void> {
  ...
  if (orphaned.length === 0) {
    ctx.ui.notify("pi-mcb: No orphaned pending files found.", "info");
    return;
  }
  ...
  if (isRpc) {
    ...
    const lines = [
      ...
      "",
      "Use /mcb cleanup in TUI mode to delete these files.",
    ];
  ...
  if (result === "deleteAll") {
    ...
    if (deleted === intended) {
      ctx.ui.notify(
        `pi-mcb: Deleted ${intended} orphaned pending file${intended === 1 ? "" : "s"}.`,
        "info",
      );
    } else {
      ctx.ui.notify(
        `pi-mcb: Deleted ${deleted}/${intended} orphaned pending file${intended === 1 ? "" : "s"} (${intended - deleted} failed).`,
        "warning",
      );
    }
    ...
    ctx.ui.notify(
      `pi-mcb: ${orphaned.length - items.length} deleted, ${items.length} remain (${(remainingSize / 1024).toFixed(1)} KB).`,
      "info",
    );
  } else if (items.length === 0 && orphaned.length > 0) {
    // All were deleted inline
    ctx.ui.notify(
      `pi-mcb: All ${orphaned.length} orphaned pending file${orphaned.length === 1 ? "" : "s"} removed.`,
      "info",
    );
  }
